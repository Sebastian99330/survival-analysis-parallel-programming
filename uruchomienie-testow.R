...
prost3 <- '"prostate_cancer_mln.csv" "time, status" "treatment" "treatment + age + sh + size + index" "'
...
lungs2 <- '"lungs-mln.csv" "time, status" "ph_karno" "age + sex + ph_ecog + ph_karno + pat_karno + meal_cal + wt_loss" "'
colon <- '"colon.csv" "time, status" "rx" "rx + sex + age + obstruct + perfor + adhere + nodes + differ + extent + surg + node4 + etype" "'
colon2 <- '"colon-mln.csv" "time, status" "rx" "rx + sex + age + obstruct + perfor + adhere + nodes + differ + extent + surg + node4 + etype" "'
flchain <- '"flchain.csv" "futime, death" "flc_grp" "age + sex + sample_yr + kappa + lambda + flc_grp + creatinine + mgus + chapter" "'
flchain2 <- '"flchain-mln.csv" "futime, death" "flc_grp" "age + sex + sample_yr + kappa + lambda + flc_grp + creatinine + mgus + chapter" "'
gbsg <- '"gbsg.csv" "rfstime, status" "hormon" "age + meno + size + grade + nodes + pgr + er + hormon" "'
gbsg2 <- '"gbsg-mln.csv" "rfstime, status" "hormon" "age + meno + size + grade + nodes + pgr + er + hormon" "'
kidney <- '"kidney.csv" "time, status" "disease" "age + sex + disease + frail" "'
kidney2 <- '"kidney-mln.csv" "time, status" "disease" "age + sex + disease + frail" "'
mgus <- '"mgus.csv" "futime, death" "pcdx" "age + sex + pcdx + pctime + alb + creat + hgb + mspike" "'
mgus2 <- '"mgus-mln.csv" "futime, death" "pcdx" "age + sex + pcdx + pctime + alb + creat + hgb + mspike" "'
myeloid <- '"myeloid.csv" "futime, death" "trt" "trt + sex + txtime + crtime + rltime" "'
myeloid2 <- '"myeloid-mln.csv" "futime, death" "trt" "trt + sex + txtime + crtime + rltime" "'
nafld1 <- '"nafld1.csv" "futime, status" "male" "age + male + weight + height + bmi" "'
nafld1_2 <- '"nafld1-mln.csv" "futime, status" "male" "age + male + weight + height + bmi" "'
nwtco <- '"nwtco.csv" "edrel, rel" "instit" "instit + histol + stage + study + age + in_subcohort" "'
nwtco2 <- '"nwtco-mln.csv" "edrel, rel" "instit" "instit + histol + stage + study + age + in_subcohort" "'
pbc <- '"pbc.csv" "time, status" "edema" "trt + age + sex + ascites + hepato + spiders + edema + bili + chol + albumin + copper + alk.phos + ast + trig + platelet + protime + stage" "'
pbc2 <- '"pbc-mln.csv" "time, status" "edema" "trt + age + sex + ascites + hepato + spiders + edema + bili + chol + albumin + copper + alk.phos + ast + trig + platelet + protime + stage" "'
